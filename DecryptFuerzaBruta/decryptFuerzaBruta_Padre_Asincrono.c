...
#define NUM_CLAVES 20 //numero de claves a generar
  ...
  switch(iId){
    ...
    //busqueda de Claves
      ...
      for(i=1; i<iNumProcs; i++){
        ...
        MPI_Isend(&mensajeNumClave, 1, MPI_UNSIGNED, i, TAG, MPI_COMM_WORLD, &request);
      }
      ...
      while (nClavesEncontradas < NUM_CLAVES)
      {
        ...
        if(strcmp(claveDesencriptada, "cancel") == 0){
          ...
          procesoDesencriptador = 0;/*proceso del que se ha recibido*/
        }
...
void printResultados(unsigned int repeticiones, double tiempoTotal, int iNumProcs){
  ...
  sprintf(filename, "%dx%d_claves%s.dat\0", NUM_CLAVES, tamanoClave, "_Padre_Asincrono");
