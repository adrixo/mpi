...
//!! A partir de aqui esta pseudocodigo, creo que falta una forma de indicar que n clave esta descifrando y los tipos de datos de comunicacion
    //busqueda de Claves
      //Iniciamos con una tarea para todos los procesos (tener cuidado si nprocesos > nClaves)
      ...
      while (nClavesEncontradas < PASSWORDS)
      {
        ...
        //1. marcamos esa contraseña como desencriptada
          //marcarDesencriptada(keyList, clavesEncontradas, claveDesencriptada, //proceso que la desencripto); //si se envia la clave en vez de n clave
          // ó dependiendo de la implementacion:
          clavesEncontradas[numClaveDesencriptada] = procesoDesencriptador; //la marcamos como
          nClavesEncontradas++;
          //manejar repeticiones

        //2. Mandamos recados los procesos que estaban con la clave desencriptada
          for(i=1; i<iNumProcs; i++)
          {
            if(procAsignadoA[i] == numClaveDesencriptada)
            {
              ...
              clavesEncontradas[ mensajeNumClave[0] ] += -1; // por optimización reducimos 1 ... vease anterior
            }
          }
        } //fi
